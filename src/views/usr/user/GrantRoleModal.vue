          ...
          >
            ...
            <template #bodyCell="{ column, record }">
              <template v-if="column.dataIndex === 'action'">
                <a-button type="dashed" :disabled="record.id==1600000000010" size="small" @click="addRecord(record)">添加</a-button>
              </template>
...
<script setup lang="ts" name="roleSelectorPlus">
  ...
  import { getRoleListByIds, getRoleOwnUser, getRoleList } from '/@/api/usr/role';
  ...
  async function handleSubmit() {
    try {
      ...
      const value = selectedData.value.map((item) => item.id);
      await grantUserRole(rowId, value);
      emit('success');
      handleClose();
    } finally {
